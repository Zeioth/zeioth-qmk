...
void pointing_device_task() {
    ...
    switch (get_highest_layer(layer_state)) {
        case _BASE:
            trackball_set_rgbw(0,0,0,32);   // White
            break;
        case _SUPER:
            trackball_set_rgbw(32,0,0,0);   // Dark red
            break;
        case _LOWER:
            trackball_set_rgbw(0,32,0,0);   // Dark green
            break;
        case _RAISE:
            trackball_set_rgbw(0,0,32,0);   // Dark blue
            break;
        case _HYPER:
            trackball_set_rgbw(32,0,32,0);  // Dark Magenta 
            break;
        case _ARROW:
            trackball_set_rgbw(32,16,16,0); // Pink 
            break;
        case _ADJUST:
            trackball_set_rgbw(32,32,0,0);  // Yellow
            break;
        default:
            trackball_set_rgbw(0,0,0,0); // Turned off
    }
